...
### ⚡ 가장 빠른 경로 (권장 순서)
1. **Supabase 프로젝트 생성** → **URL·anon 키** 2개만 확보 (§1) — 이것만 해도 **이메일 회원가입/로그인이 실제로** 됩니다.
2. **구글** 붙이기 (§3) — 클라이언트 전용이라 제일 쉬움.
3. **카카오** 붙이기 (§4).
4. (선택) **동기화 스키마** 실행 (§5.5).
>
> 💡 **URL·anon 키는 공개용**이라 저에게 알려주시면 제가 `auth-config.js`에 넣고 배포해 드립니다. 구글·카카오의 **Secret은 Supabase 대시보드에만** 넣으면 되고 저에게 주실 필요 없습니다.
> 💡 소셜 로그인의 **회원/입점사 역할**은 코드가 자동 처리합니다(로그인 직전 의도한 역할을 저장 → 복귀 후 계정에 1회 반영). 별도 설정 불필요.
